...
WMap.prototype = {
    ...
    addInteraction: function (type, layers, callback) {
        var mapType = this.mapType,
            map = this.map;
        var orglayers = layers.map(layer => {
        ...
        if ('openlayers' === mapType) {
            ...
            layers.forEach(value => {
                ...
                value.getFeatures().forEach(feature => {
    ...
    /**
     * 地图点击事件
     * @param type {String} 支持click,dbclick,rightclick,rightdblclick
     * @param callback {Function}
     */
    mapClick: function (type, callback) {
        var mapType = this.mapType,
            map = this.map;
        if ('openlayers' === mapType) {
    ...
    moveEnd:function(callback){
        var mapType = this.mapType,map = this.map;
        if('openlayers'===mapType){
...
WLayer.prototype = {
    ...
    addFeature: function (feature) {
        ...
        if ('openlayers' === mapType) {
            ...
            if (Object.keys(_this.interactions).length) {
                Object.keys(_this.interactions).forEach((value) => {
                    _this.map.addFeaturesInteraction(value, [feature], _this.interactions[value]);
                })
            }
        }
    },
    ...
    addFeatures: function (features) {
        ...
        if ('openlayers' === mapType) {
            ...
            features.forEach((value, key) => {
                ...
                //解决wmap添加interaction在wlayer添加feature之前时，无法给后添加的feature绑定事件的bug
                if (Object.keys(_this.interactions).length) {
                    Object.keys(_this.interactions).forEach(item => {
                        _this.map.addFeaturesInteraction(item, [value], _this.interactions[item]);
                    })
                }
            })
        }
    ...
    hide: function () {
        ...
        if ('openlayers' === mapType) {
            ...
            this.features.show().forEach((value, key) => {
                value.hide();
            })
        }
    },
...
WFeature.prototype = {
     ...
     * @param {Array.<Number>} offset 图片相对锚点的偏移
     * @returns {any} feature openlayers为ol.Feature， baidu为BMap.Marker
     */
    Icon: function (coordinates, url, size, anchor, offset) {
        var mapType = this.mapType;
        if ('openlayers' === mapType) {
            ...
            if (offset) {
                icon.setOffset(new BMap.Size(offset[0], offset[1]));
            }
        }
        return icon;
    },
    Polyline: function (coordiantes, strokeColor, strokeWeight) {
        ...
        if ('openlayers' === mapType) {
            ...
            var bcoordinates = coordiantes.map(value => {
     ...
     * @param {Bealean} enableClicking 是否启动响应点击事件
     */
    Polygon: function (coordiantes, strokeColor, strokeWeight, fillColor, enableClicking) {
        var mapType = this.mapType;
        if ('openlayers' === mapType) {
    ...
    TextIcon: function (coordinates, textOpts, url, size, anchor, opts) {
        ...
        if ('openlayers' === mapType) {
            ...
            label.setStyle({ border: "solid 0px black", color: textOpts.color || "red", backgroundColor: textOpts.backgroundColor||"transparent",width:textOpts.width||"120px",textAlign:textOpts.textAlign||"center" });
...
export function WOverlay(map, opts) {
    ...
    if ('openlayers' === this.mapType) {
        ...
        var infoBox = new BMapLib.InfoBox(this.map, "", opts || {
            ...
            closeIconUrl: "/static/assets/map/bd_close.png",
            enableAutoPan: true,
...
WDrawingManager.prototype = {
    ...
    setDrawingMode: function (DrawingType, callback) {
        ...
        if ('openlayers' === mapType) {
            _this.openlayerDrawingJson.forEach((value, key) => {
    ...
    close: function () {
        ...
        this.status = false;
        if ('openlayers' === mapType) {
            this.openlayerDrawingJson.forEach((value, key) => {
